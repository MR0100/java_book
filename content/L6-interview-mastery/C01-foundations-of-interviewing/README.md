---
title: "Interview Mastery (FAANGM + MNC) — Foundations of Interviewing"
slug: l6-foundations-of-interviewing
level: L6
module: "Interview Mastery (FAANGM + MNC)"
section: "Foundations of Interviewing"
type: index
difficulty: senior
order: 1
tags: [section-index]
prerequisites: []
status: planned
estimated_minutes: 3
last_updated: 2026-06-09
---

# Interview Mastery (FAANGM + MNC) — Foundations of Interviewing

This section covers the topics below. Each `.md` file is created as the topic is authored — see [CONVENTIONS](../../../templates/CONVENTIONS.md).

| # | Topic | Planned file | Status |
|---|-------|--------------|--------|
| 01 | How tech interviews & leveling work (MNC vs FAANGM) | `T01-how-tech-interviews-and-leveling-work-mnc-vs-faangm.md` | planned |
| 02 | The interview funnel — recruiter, screen, loop, debrief, offer | `T02-the-interview-funnel-recruiter-screen-loop-debrief-offer.md` | planned |
| 03 | The interviewer's rubric — signals, scoring, calibration | `T03-the-interviewer-s-rubric-signals-scoring-calibration.md` | planned |
| 04 | Big-O / time & space complexity | `T04-big-o-time-and-space-complexity.md` | planned |
| 05 | Communication mechanics — clarify, structure, think-aloud, recover | `T05-communication-mechanics-clarify-structure-think-aloud-recover.md` | planned |
| 06 | Prep system — weeks-out plan, mock cadence, day-of routine | `T06-prep-system-weeks-out-plan-mock-cadence-day-of-routine.md` | planned |

[Back to L6 index](../README.md) · [Master curriculum](../../../docs/CONTENTS.md)
