...
Run via the wrappers (`scripts/audio/build.sh` on macOS/Linux,
`scripts/audio/build.ps1` on Windows). Direct invocation works too:

    python scripts/audio/build.py L0/C01            # scope to one chapter
    python scripts/audio/build.py L0                # one module
    python scripts/audio/build.py                   # whole course
    python scripts/audio/build.py --force L0/C01    # regenerate
    python scripts/audio/build.py --diff L0/C01     # dry-run, list what would change

Output goes to the repo-root `audio/` folder (auto-created, git-ignored).
"""
...
THIS = Path(__file__).resolve()               # scripts/audio/build.py
PROJECT_ROOT = THIS.parents[2]                 # repo root  (audio → scripts → repo)
CONTENT_DIR = PROJECT_ROOT / "content"
# `audio/` is a purely auto-generated, git-ignored output folder, created on the
# first build. Its layout mirrors `content/` so the web builder can find a topic's
# MP3 by the same relative path and show an optional audio player.
OUT_DIR = PROJECT_ROOT / "audio"
MANIFEST = OUT_DIR / "manifest.json"
PREPROCESS = THIS.parent / "preprocess.py"
PIPER_VOICES_DIR = OUT_DIR / "voices" / "piper"
